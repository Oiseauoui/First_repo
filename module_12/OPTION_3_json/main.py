...
def save_contacts(contacts_dict):
    ...
    # Define the file path for contacts.json in the same directory
    json_file_path = os.path.join(script_dir, 'contacts.json')

    # Create a list to store contact data as dictionaries
    contacts_data = []
    ...
    with open(json_file_path, 'w') as json_file:
        json.dump(contacts_data, json_file)
...
def main():
    json_file_path = r"E:\PYTHON\REPOSITORY PYTHON\First_repo\module_12\contacts.json"  # Задайте шлях до файлу JSON тут
    contacts_dict = AddressBook()

    # Завантажити контакти з файлу JSON, передаючи шлях до файлу
    load_contacts(contacts_dict, json_file_path)
